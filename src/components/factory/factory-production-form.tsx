...
export function FactoryProductionForm() {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    if (!productionDate) return setMessage({ type: 'error', text: 'Production Date is required.' });
    ...
    try {
      await mutation.mutateAsync({
        ...
        occurred_at: productionDate,
      });
      setMessage({ type: 'success', text: 'Production recorded successfully.' });
      setRawMaterial(''); setBatchReference(''); setWeight(''); setTemperature(''); setHours(''); setMinutes(''); setProductionDate(today()); setProductId(''); setOutputQuantity('');
    } catch (error) {
  ...
  return (
    <form onSubmit={submit} className="space-y-5 rounded-3xl border border-slate-200 bg-white p-6 shadow-soft">
      ...
      <fieldset className="space-y-2"><legend className="text-sm text-slate-700">Processing Duration</legend><div className="grid gap-4 sm:grid-cols-2"><label className="block text-sm text-slate-700">Hour<input type="number" min="0" step="1" value={hours} onChange={(event) => setHours(event.target.value)} className="mt-2 w-full rounded-3xl border border-slate-200 bg-slate-50 px-4 py-3 outline-none" /></label><label className="block text-sm text-slate-700">Minute<input type="number" min="0" max="59" step="1" value={minutes} onChange={(event) => setMinutes(event.target.value)} className="mt-2 w-full rounded-3xl border border-slate-200 bg-slate-50 px-4 py-3 outline-none" /></label></div></fieldset>
      <label className="block text-sm text-slate-700">Production Date<input type="date" required value={productionDate} onChange={(event) => setProductionDate(event.target.value)} className="mt-2 w-full rounded-3xl border border-slate-200 bg-slate-50 px-4 py-3 outline-none" /></label>
